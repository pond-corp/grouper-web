...
export function days_between_dates(date1: Date, date2 = new Date()): number {
    const y2 = date2.getUTCFullYear(), m2 = date2.getUTCMonth(), d2 = date2.getUTCDay();
    let y1 = date1.getUTCFullYear(), m1 = date1.getUTCMonth(), d1 = date1.getUTCDay();
    let days = 0;
...
export function unpack_date_bigint(date_bigint: bigint): Date {
    return new Date(Number(date_bigint >> 21n), Number(((date_bigint >> 17n) & 7n) + 1n), Number((date_bigint >> 12n) & 31n), Number((date_bigint >> 6n) & 63n), Number(date_bigint & 63n));
}
/**
